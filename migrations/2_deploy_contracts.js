...
var dummyalgorithm = artifacts.require(
  '@ensdomains/dnssec-oracle/DummyAlgorithm.sol'
);
var dummydigest = artifacts.require(
  '@ensdomains/dnssec-oracle/DummyDigest.sol'
);
var _DNSSECInterface = artifacts.require(
  '@ensdomains/dnssec-oracle/DNSSEC.sol'
);
var DNSSEC = artifacts.require('@ensdomains/dnssec-oracle/DNSSECImpl.sol');
var rsasha1 = artifacts.require(
  '@ensdomains/dnssec-oracle/RSASHA1Algorithm.sol'
);
var rsasha256 = artifacts.require(
  '@ensdomains/dnssec-oracle/RSASHA256Algorithm.sol'
);
...
var nsec3sha1 = artifacts.require(
  '@ensdomains/dnssec-oracle/SHA1NSEC3Digest.sol'
);
var DNSRegistrar = artifacts.require(
...
module.exports = function(deployer, network) {
  ...
  return deployer
    .deploy(DNSSEC, dnsAnchors.encode(anchors))
    .then(() =>
      deployer.deploy([
        [ENSRegistry],
        [rsasha256],
    ...
    .then(() =>
      DNSRegistrar.deployed().then(_registrar => (registrar = _registrar))
    )
    .then(() =>
      ENSRegistry.deployed().then(_ens =>
        _ens.setSubnodeOwner(0, sha3(tld), registrar.address)
      )
    )
    .then(() =>
      DNSSEC.deployed().then(_dnssec => {
        tasks = [];

        tasks.push(
          rsasha1.deployed().then(async function(algorithm) {
        ...
        tasks.push(
          rsasha256
            .deployed()
            .then(algorithm => dnssec.setAlgorithm(8, algorithm.address))
        );
        tasks.push(
          sha1.deployed().then(digest => dnssec.setDigest(1, digest.address))
        );
        tasks.push(
          sha256.deployed().then(digest => dnssec.setDigest(2, digest.address))
        );
        tasks.push(
          nsec3sha1
            .deployed()
            .then(digest => dnssec.setNSEC3Digest(1, digest.address))
        );
