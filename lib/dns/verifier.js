...
function groupByNameAndType(list){
  const groupedByName = _.groupBy(list, (n) => { return n.name})
  const groupedByType = _.groupBy(list, (n) => { return n.type})
  const grouped = {}
  Object.keys(groupedByType).map(g => {
    if (g !== 'RRSIG' && g !== 'SOA'){
      if(!grouped[g]){
        grouped[g] = {}
      }
      let values = groupedByType[g]
      let rrsetByName = _.groupBy(values, (n) => { return n.name})
      Object.keys(rrsetByName).map(name => {
        if(!grouped[name]){
          grouped[g][name] = {}
        }
        if(g === 'NSEC3'){
          grouped[g][name].rrs = [rrsetByName[name][0]]; // This is to take whatever the first one
          grouped[g][name].sigs = [filterRRSIGWithTypeCovered(groupedByName[name], [g])[0]]; // This is to take whatever the first one
        }else{
          grouped[g][name].rrs = rrsetByName[name];
          grouped[g][name].sigs = filterRRSIGWithTypeCovered(groupedByName[name], [g])  
        }
      })
    }
  })
...
async function queryWithProof(qtype, name) {
  ...
  if(r.authorities.length > 0){
    console.log('*** queryWithProof', {authorities:r.authorities})
    groups = groupByNameAndType(r.authorities)
  }else{
    console.log('*** queryWithProof', {answers:r.answers})
    groups = groupByNameAndType(r.answers)
  }
...
for (const k of Object.keys(groups)) {
  for (const n of Object.keys(groups[k])) {
    let set = groups[k][n]
    console.log('*** ret2 1', {set})
    ret2 = await verifyRRSet(set.sigs[0], set.rrs);
    if (ret2) {
      ret2.push(new SignedSet(n, set.sigs[0], set.rrs));
      console.log('*** ret2 2', {ret2})
    }
  }
}
console.log('*** ret2 3', {ret2})
return ret2
//  let s = new SignedSet(name, sigs, rrs)
//  let obj = { 'NSEC3': { 'a.matoken.xyz':s }
// Object.keys(groups).map(k => {
//   Object.keys(groups[k]).map(n => {
//     let set = groups[k][n]
//     ret2 = await verifyRRSet(set.sigs, [set.rrs]);
//     if (ret2) {
//       ret2.push(new SignedSet(name, sig, rrs));
...
// })
  ...
  //   console.log('*** verifyRRSet', {sig, rrs})
...
async function getDNS(buf) {
  let response = await fetch(
    DNS_URL + buf.toString('base64') + '&ts=' + Date.now()
  );
  let buffer;
