...
async function queryWithProof(qtype, name) {
  ...
  let ret, nsec, found;
  if(rrs.length > 0){ found = true;};

  if(r.authorities.length > 0){
    sigs = filterNSECRRSIG(r.authorities);
    rrs = filterNSEC(r.authorities);
    if(rrs.length > 0){ nsec = true;};
  }
  if(!found && !nsec){
    return{
      found: found,
...
function filterRRs(rrs, qtype) {
  return rrs.filter(r => {
    return r.type == qtype;
  });
}

function filterNSEC(rrs, qtype) {
  return rrs.filter((a)=>{ return a.type == 'NSEC' || a.type == 'NSEC3' })
}

function filterNSECRRSIG(rrs) {
  return rrs.filter((a)=>{ return a.type == 'RRSIG' && a.data.typeCovered == 'NSEC' })
}
...
async function getDNS(buf) {
  let response = await fetch(DNS_URL + buf.toString('base64'));
  let buffer;
