...
function pack(rrset, sig) {
  ...
  let rrdata
  if(rrset[0].type === 'NSEC3'){
    // NSEC3 returns 3 records and we only need the second (rrset[1]) nsec name hash which comes after the name to delete.
    // https://github.com/ensdomains/dnssec-oracle/blob/3afc8e5d53fe964a18ac8eb4ee085419fc7096aa/contracts/DNSSECImpl.sol#L258
    // https://tools.ietf.org/html/rfc7129#section-5.5
    rrdata = rawSignatureData([rrset[1]], sig);
  }else{
    rrdata = rawSignatureData(rrset, sig);
  }
