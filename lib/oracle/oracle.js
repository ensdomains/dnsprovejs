...
class Oracle {
  ...
  async submitEach(result, params) {
    let proven = await this.getProven(result);
    await this.submitInternal(result.proofs, proven, params);
  }
  ...
  async submitInternal(proofs, idx, params) {
    await this.submitProof(
      proofs[idx],
      proofs[idx - 1],
      Object.assign({}, params)
    );
    if (idx + 1 < proofs.length) {
      await this.submitInternal(proofs, idx + 1, params);
    }
  }
}
