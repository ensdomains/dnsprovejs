...
class Oracle {
  ...
  async knownProof(proof) {
    ...
    let toProve = this.web3.utils.sha3('0x' + proof.rrdata.toString('hex'), {encoding:"hex"}).slice(0,42)
    let matchedHash = (hash === toProve); 
    return({
      inception:inception,
      ...
      matched:(validInception && matchedHash)
    });
  }
