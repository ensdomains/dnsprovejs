...
class Oracle {
  ...
  async knownProof(proof) {
    ...
    console.log('knownProof', 'proof.name', proof.name, 'name', name, 'type', type , new Date());
  ...
  async submitProofs(proofs, proven, params) {
    ...
    if (proven == 0) {
      ...
      // TODO: This path is not working event.
      // prevProof = proofs[proven -1].toSubmit();
      prevProof = proofs[proven -1].toConcat().toString('hex');;
    }
    ...
    let trx = await this.contract.methods.submitRRSets(data, prevProof).send(params);
    trx.events['RRSetUpdated'].forEach((e, i)=>{ console.log(i, e)})
    return true;
  }
