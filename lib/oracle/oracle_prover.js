class OracleProver {
  constructor(oracle, total, proven, proofs) {
    this.oracle = oracle;
    this.total = total;
    this.proven = proven;
  ...
  async submitOnce(params) {
    await this.oracle.submitProofs(this.proofs, this.proven, params)
  }

  async submitInternal(idx, params) {
    await this.oracle.submitProof(
      this.proofs[idx],
      this.proofs[idx - 1],
