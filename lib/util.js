...
function checkDigest(anchor, name, input, digestType) {
  let digest
  var encoded = packet.name.encode(name);
  console.log('checkDigest1', {anchor, name, input, digestType, encoded})
  switch (digestType) {
    case 1:
    case 2:
      digest = jssha1(
      ...
      console.log('checkDigest2', {digestType, dugest})
      return digest.equals(anchor.data.digest);
    case 8:
    case 13:
      digest = ethereumUtil.sha256(
        Buffer.concat([packet.name.encode(name), input])
      );
      console.log('checkDigest3', {digestType, dugest})
      return digest.equals(anchor.data.digest);
